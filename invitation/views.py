...
def invite(request, success_url=None,
    ...
    **Context:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                invitation = Invitation.objects.invite(request.user, form.cleaned_data["email"])

                # Chequear si existe invitación , registro solo por invitación
                if get_user_model().objects.filter(email=invitation.email).count() > 0:
                    messages.add_message(request, messages.ERROR, "El e-mail ya está registrado.")
            ...
            messages.add_message(request, messages.INFO, "Invitación realizada exitósamente.")
